...
| `tools/check_icon_sync.py` | Refuses a pull request that edits a source SVG in `resources/` and leaves the icons generated from it behind. Proves they moved together, not that they are correct — nothing re-runs `build_icons.py`, so a wrong regeneration passes. An edit that renders identically leaves nothing to commit; say `icons-unchanged: <reason>` in a commit message. |
