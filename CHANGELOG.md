...
- **Two stores could be given the same colour, and the legend exists to tell
  ...
  hash still assigns every colour; only a store that lands on one already taken
  moves, so a store that was not part of the problem keeps the colour it has
  always had. Where it moves to is measured rather than arbitrary: two of the
  six hues are effectively identical to a reader with red-green colour
  blindness, so simply picking a different slot could have changed nothing for
  roughly one person in twelve while looking fixed. Colour separates confidently
  at two and three stores and is normal-vision-only above that; past six there
  is nothing left to give and collisions return. The label sits beside every
  colour at every count, which is why this degrades rather than breaks.
