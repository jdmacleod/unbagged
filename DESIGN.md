...
**Red is retired** to three call sites, and it means *this is not recoverable*:
confirming deletion of an imported report, a request that failed, and a view
that could not be drawn. Nothing else, ever. It previously fired thirty times in
a single column, which is how it stopped meaning anything. The fix was never a
different hue — it was deleting the severity. Grocery prices rising is the
subject matter, not an alarm.

Count them before adding a fourth. Three is already a claim about scarcity that
