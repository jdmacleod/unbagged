...
Typography above is the faces. This is what gets set in them.

**American English, in what a reader sees.** The app is read by people filing
right-to-know requests with American grocers, and "at the till" or "the weekly
shopping" reads as somebody else's country. The scope is deliberately narrow:
**reader-facing strings only**. Identifiers, wire-format fields and the spellings
in comments and docs are out, and `itemised_count` is the reason the line is
drawn there — it is a field name in `views.py` and in `types.ts`, so "American
English" without a scope is an invitation to rename an API. This file and
`CLAUDE.md` are themselves written in British English and stay that way.

**A line holds about 80 characters.** Prose caps at 62ch (see Layout), and 62
zero-widths is roughly 80 characters of ordinary lowercase, because `ch` and the
glyphs scale together — measured at 82 on the 13px sans lede and 80 on the
11.5px marginalia, which is the same budget twice rather than two budgets. So a
sentence of 80 characters sets on one line at any size the app uses.

**Two lines is a choice.** Wrapping is not a defect — the 62ch cap exists to
stop a line running wider than the eye tracks comfortably, and the empty half of
the sheet beside a paragraph is the margin doing its job. What is a defect is a
sentence that runs to 83 characters and takes a second line to carry three
words. Count before writing a lede: land under 80 or commit to the second line
and use it.

**One idea per sentence, and cut the throat-clearing.** "That is not the same as
having made no purchases" is the app doing its most important work and earns its
length. "It is read here, on this machine, and nothing is uploaded anywhere"
...
| `--ink-danger` | `#8C2F22` | two render sites, three meanings — see Red |
...
**Red is retired** to three meanings, and every one of them is *this is not
recoverable*: confirming deletion of an imported report, a request that failed,
and a view that could not be drawn. Nothing else, ever. It previously fired
thirty times in a single column, which is how it stopped meaning anything. The
fix was never a different hue — it was deleting the severity. Grocery prices
rising is the subject matter, not an alarm.

**Two counts, and they are not the same number.** Two things render
`--ink-danger`: `RemoveRequest.tsx` and `ui.tsx::ErrorBox`. Those two carry the
three meanings between them, because `ErrorBoundary` reuses `ErrorBox` rather
than drawing its own red. Count the meanings before adding a fourth — not the
render sites. The render count sat still at two while the third meaning arrived
through a tenth consumer of a shared component, so a grep for the token reports
a contract that has already moved. Three is already a claim about scarcity that
the next person will test.
...
|---|---|---|
| 2026-09-18 | A product is shown under a cleaned label, and a line that is only a charge is not shown at all | The retailer prints its own furniture into the name field, and the reader was seeing it: a promotional price standing where a product name goes, a leading bracket, and in one case a single copyright glyph as a whole product. Three rules, each drawn tight against a measured false positive. **The price strip requires a currency symbol.** Stripping any leading number was measured against the corpus and matched seven names, five of them prices and two a bottle size followed by a unit; eating the size leaves a name that is wrong and still reads like a product, which is the `2% MILK` hazard one step on. **A line is refused only when EVERY word is deposit or tax vocabulary.** Containment matching takes eight rows where five are meant, and the three it takes are real drinks carrying the word beside a size — the same shape as `_is_furniture` deleting a real `GIFT CARD` by matching `TENDER` alone. **Where cleaning would make two rows read alike, neither is cleaned.** They are two products with two keys, and one name twice in an alphabetical list reads as double counting. Appending the code instead was rejected: the 2026-09-15 row above forbids exactly that swap in the mirror case, and `description` is also what `export.ts` sets in the serif inside the downloadable index poster. **The count says what it dropped.** `set_aside` is rendered under the headline, because a total that shrinks with nothing on screen to account for it is the one thing this app must never do; `stats` now takes `distinct_products` from the index rather than counting in SQL, so the figure and the page cannot disagree |
| 2026-09-18 | A Words section, and American English scoped to what a reader sees | Typography governed the faces and nothing governed the sentences, so register drifted: four British idioms reached shipped copy (`at the till`, `the shopping` twice, `bought in from`) and nobody had a rule to catch them. The scope is the load-bearing half. `itemised_count` is a field name in `views.py` and `types.ts` AND the word a reader sees in the timeline, so a rule reading "American English" without a boundary invites the next contributor to rename an API field; the boundary is reader-facing strings, and identifiers, wire-format fields and comment spellings are explicitly out. This file stays in British English. **The line budget is one number, not two.** Prose caps at 62ch and 62 zero-widths is about 80 characters of lowercase, measured at 82 on the 13px sans lede and 80 on the 11.5px marginalia — `ch` and the glyphs scale together, so it is the same budget at every size. The app's own header ran to 83 and wrapped by a single character, which is what a budget is for. Wrapping itself is NOT the defect and the 62ch cap is not under review: the empty half-sheet beside a paragraph is the margin working as the 2026-09-03 row intended |
| 2026-09-15 | A response may itemise SOME of its visits; the timeline says which fraction, and a product needs a name rather than a code | `lines_disclosed` is one bit for a whole response and there are three states, not two. H Mart answered twice — a points statement covering all 67 visits, then screen captures of the receipts covering 43 of them — and on that response the bit reads true, so the paragraph explaining why rows do not open disappeared and two dozen of them became controls that silently cannot pay out. That is the exact failure the paragraph was written to fix, reached from the other side: it was conditioned on the absence being total. The third state names the fraction and says which rows the missing half belongs to, so a reader can stop trying them. **Its numerals are prose, not mono** — caught in review: the first version set them in `.num`, and the row you are reading claimed the numeral rule required that. It requires the opposite. Numerals in running prose stay in the serif with tabular-nums; the mono is for numerals in an ALIGNED context, and nothing aligns with a sentence. Three other counts in this same file already render bare — `FootingNote`'s "N of M baskets do not add up", `Arrival`'s "Showing N of M visits", and the same shape twice in PriceHistory — so the mono version was the odd one out and the log entry defending it made this file disagree with its own Typography section. **No colour and no mark**: the rows that do not open are already distinguished by not opening, and status is carried by typography and layout here as everywhere else. Rejected: a per-row marker, which is the thirty-uses-of-red shape — one glyph on 24 rows to say what one sentence says once. Second half of the same decision: a product is identified by the retailer's code where one was disclosed and by the name it printed where none was. Both product views keyed on a UPC, and these receipts carry none, so Products and Prices rendered empty pages over 388 disclosed line items — the tool answering "nothing" to the one question this response had answered. A name is the weaker key and the weakness is the retailer's: two products whose names truncate alike merge, one renamed between visits splits, and both are visible to a reader looking at the list, which showing nothing is not. The code stays a separate field and is still set in the mono stack — so where there is none the slot is **absent rather than filled with the name**, which would be the same word twice, the second time in a face chosen for digits. |
| 2026-09-11 | A view that could not be drawn is the third red call site; the count in this section moves from two to three | `ErrorBoundary` (issue #49) renders the same `ErrorBox` as a failed request, and a render throw is neither of the two meanings this section enumerated: the request succeeded and the drawing failed. **The component count never moved, which is exactly why this was easy to miss** — `RemoveRequest.tsx` and `ui.tsx::ErrorBox` are still the only two things that render `--ink-danger`, so a grep says the contract holds while a third meaning has already been added through a tenth consumer of a shared component. That is the drift that took red to thirty uses, one reasonable reuse at a time, so it is written down rather than absorbed. *Not recoverable* still holds, read as "you cannot fix this from here": a reader can retry a failed request but cannot fix the server, and can reload past a render throw but cannot fix the shape that broke it. The boundary's own copy — the response is still stored and nothing has been lost — is set in ink beside the mark rather than in red, because it describes the data and the mark describes the fault. Rejected: a non-red treatment for the boundary, which would have held the count at two by making the one screen in the app a reader genuinely cannot recover from look like a caveat. **This section now states two counts rather than one** — two render sites, three meanings — because a single number cannot be both checked by grep and trusted as the contract, and collapsing them is what let the third meaning in. Count the meanings before adding a fourth. Caught in review on #79, along with the token table still reading "exactly two call sites" and three code comments still describing red as one or two: the 2026-09-09 row below records the table lagging the prose the last time this count moved, and it lagged again the same way, so the table now names both numbers instead of one |
