...
4. **Identity.** A categorical palette of six hues (`--cat-1` … `--cat-6`), used
   ...
   **One exception, and only one:** when two keys in the same response hash to
   the same hue, position breaks the tie — the later key by sort order takes
   the next free hue. Nothing that does not collide ever moves. See the
...
|---|---|---|
| 2026-09-10 | The identity hash keeps assigning; when two keys in one response collide, the later one by sort order moves to the nearest free hue in a measured spread order | The Identity rule above holds unchanged for every store that does not collide. Six buckets means three stores share a hue about 44% of the time, and the H Mart response hits it: two branches both resolved to `--cat-4`, defeating the legend's only job on the one response class where store is the only categorical dimension in the app. Assigning by position across the whole set would trade away the cross-response recognition the hash exists for; breaking ties by position trades it only for the pair that was already broken. The tiebreak target is not arbitrary. Measured on this palette as CIEDE2000 under Viénot 1999 dichromat simulation, `--cat-2` and `--cat-4` are ΔE 1.1 apart under deuteranopia and `--cat-2`/`--cat-6` are ΔE 2.8 apart under protanopia — indistinguishable at the 8px dot the timeline draws — so a de-collision that merely avoids reusing an index can change nothing at all for roughly 1 in 12 men while its test passes on the numbers differing. The order `[3,4,1,6,5,2]` maximises the worst pairwise separation and comes out identical in light and dark. What it buys, worst pair across normal, deuteranopic and protanopic vision: 2 keys ΔE 43.7 · 3 keys 14.4 · 4 keys 6.0 · 5 keys 4.8 · 6 keys 1.1. So colour separates confidently at two and three, is normal-vision-only from four to six, and past six there is no free hue and the hash's collisions stand. The label carries the meaning at every count, which is why this degrades rather than breaks — and why the ladder is written down rather than the palette being claimed to scale |
