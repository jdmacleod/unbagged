...
negative amounts, the missing sections, and the three footing outcomes.

Scale differs from the reference report and that is deliberate — the fixture is
seeded from structure, never from values. It currently yields 120 baskets, 1,740
line items and 425 distinct UPCs across 24 months, against 54 baskets and 790
line items in the report that was read. Prices carry a small annual drift so the
price-history view shows a series rather than noise.

Where it reproduces a **distribution** rather than a shape, the measured figure
is the target and the fixture is checked against it:

| | fixture | response |
|---|---|---|
| baskets footing exactly | 81 of 120 (68%) | 34 of 54 (63%) |
| lines exceed the stated total | 2 | 2 |
| lines fall short of it | 37 (31%) | 18 (33%) |
| lines at the shelf amount | 568 (66%) | 517 (66%) |
| lines below it | 287 | 271 |
| lines above it | 0 | 0 |
| `customerloyamt` zero, `retailamt` positive | 38 | 35 |
| summed loyalty ÷ summed shelf | 88.8% | 88% |

The last row is the one to watch, because it is the figure a reader actually
reads off the Timeline. The two above it can each be right while it is wrong: a
mostly-full-price fixture with shallow discounts reports a card worth 4%, and
one with deep discounts on a third of lines reports 15%.
